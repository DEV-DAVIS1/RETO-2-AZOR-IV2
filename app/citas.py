...
def verificar(texto: str, fragmentos: list[dict]) -> dict:
    """Cuantas citas del texto existen entre los fragmentos recuperados.

    Se llama con el texto CRUDO del modelo, antes de `aplicar()`: este elimina las
    citas inventadas sin dejar rastro, y el dato de cuantas hubo es util para
    auditar el sistema. Un chunk_id que no esta entre los recuperados es una
    alucinacion detectable sin ambiguedad (idea del notebook, `verificar_citas`).
    """
    citados = list(dict.fromkeys(PATRON_ID.findall(texto or "")))
    reales = {str(f.get("chunk_id")) for f in fragmentos or []}
    validos = [c for c in citados if c in reales]
    return {
        "citas_totales": len(citados),
        "citas_verificadas": len(validos),
        "citas_descartadas": [c for c in citados if c not in reales],
        "tasa_verificacion": round(len(validos) / len(citados), 3) if citados else None,
        "fragmentos_recuperados": len(reales),
    }
